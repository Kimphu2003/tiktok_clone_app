...
class VideoController extends GetxController {
  ...
  _updateVideoFavoriteCount(String videoId, bool isFavorite) async {
    try {
      ...
      if(videoDoc.exists) {
        int currentCount = (videoDoc.data()! as dynamic)['favoriteCount'] ?? 0;
        int newCount = isFavorite ? (currentCount - 1) : (currentCount + 1);
        await fireStore.collection('videos').doc(videoId).update({
          'favoriteCount': newCount,
        });
        debugPrint('updated video favorite count successfully');
      } else {
        debugPrint('Video document does not exist');
      }
    } catch(e) {
